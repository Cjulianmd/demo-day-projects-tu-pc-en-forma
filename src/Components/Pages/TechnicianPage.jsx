...
function TechnicianPage() {
  ...
  useEffect(() => {
    getDocs(
      query(collection(db, "Citas"), where("estado", "==", "creada"))
    ).then((response) => {
      let listaCitas = [];
      response.forEach(
        (doc) => {
    ...
    getDoc(doc(db, "Tecnicos", tecnico.id)).then((response) => {
      let dataTecnico = response.data();
      getDocs(
        query(
          collection(db, "Citas"),
          where(documentId(), "in", dataTecnico.citas)
        )
      ).then((response) => {
        let listaCitas = [];
        response.forEach(
  ...
  const escogerCita = (index) => {
    updateDoc(doc(db, "Tecnicos", tecnico.id), {
      ...
      updateDoc(doc(db, "Citas", citasL[index].DNI), {
        estado: "agendada",
  ...
  const completarCita = (index) => {
    updateDoc(doc(db, "Tecnicos", tecnico.id), {
      citas: arrayRemove(citasT[index].DNI),
    }).then(() => {
      deleteDoc(doc(db, "Citas", citasT[index].DNI)).then(() => {
